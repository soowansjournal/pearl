...
for game_ind in range(len(op_games)):
    ...
    for mmdd_p in mmdd_p_all:
        ...
        # 3) Go through Ajmal's Results File
        if leg_side == 'R':
            folder_path = '/Users/soowan/Documents/PEARL/Data/Data_OB2/Results_Ajmal/SLS/SingleR'
        else:
            folder_path = '/Users/soowan/Documents/PEARL/Data/Data_OB2/Results_Ajmal/SLS/SingleL'
        ...
        for filename in os.listdir(folder_path):
            ...
            if os.path.isfile(file_path):
                ...
                if mmdd_p[-3:] in filename and op_games[game_ind] in filename:
                    ...
                    if 'OP' in filename:
                        print(filename)
                        cols_to_load = ['StartTime', 'EndTime', 'TimerStart', 'TimerEnd', 'RightSide', 'QualityRep']
                        op = pd.read_csv(file_path, usecols=cols_to_load)

                        if leg_side == 'R':
                            for i in range(1,len(op)):
                                if op['QualityRep'][i] == True and op['RightSide'][i] == True:
                            ...
                            for i in range(1,len(op)):
                                if op['QualityRep'][i] == True and op['RightSide'][i] == False:
                                    time = op['TimerEnd'][i] - op['TimerStart'][i]
                                    op_sum_time.append(time)
                                elif op['QualityRep'][i] == False: 
                                    time = 0
                    ...
                    if 'MA' in filename:
                        ...
                        if leg_side == 'R':
                            for i in range(1, len(ma)):
                                if ma['QualityRep'][i] == True and ma['RightSide'][i] == True:
                            ...
                            for i in range(1, len(ma)):
                                if ma['QualityRep'][i] == True and ma['RightSide'][i] == False:
                                    time = ma['TimerEnd'][i] - ma['TimerStart'][i]
                                    ma_sum_time.append(time)
                                elif ma['QualityRep'][i] == False: 
                                    time = 0
    ...
    if leg_side == 'R':
        df_game.to_csv(rf'/Users/soowan/Downloads/2023-{op_games[game_ind]}-Right-TIMER.csv', encoding = 'utf-8-sig')
    else: 
