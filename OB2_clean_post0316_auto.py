'''
---
# **Copy of CHAPTER 3: OB2 Clean Post0316 Auto**
---
...
from OB2_clean_post0316_fun_auto import * # todo import other modules
    ...
    for i in range(len(peaks)):
        if peaks["Date_P##"][i] == mmdd_p:
          if str(peaks[op_games[game_ind]][i]) == 'nan':
              ...
              # Change Resulting Structure to Fit Ajmal's Algorithm
              # standardize the time column
              first_value = op_final.iloc[0,0]
              op_final['Time'] = op_final['Time'] - first_value
              first_value = ma_final.iloc[0,0]
              ma_final['Time'] = ma_final['Time'] - first_value

              # remove the index column
              op_final = op_final.set_index('Time')
              ma_final = ma_final.set_index('Time')

              # does ajmals algorithm account the coordinate transform?
